...
export default function BookingForm({ doctor, onSubmit }: { doctor: Doctor; onSubmit: (data: { date: string; time: string; notes: string }) => void }) {
	return (
		<form
			onSubmit={e => {
				e.preventDefault();
				...
				onSubmit({ date: data.date as string, time: data.time as string, notes: (data.notes as string) || '' });
			}}
